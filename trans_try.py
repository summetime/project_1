...
# sentences = [
#     # 中文和英语的单词个数不要求相同
#     #dec_input dec_output decode的输入和输出，主要用于训练
...
# src_vocab=utils.word2indexofenc('enc_input.en')
# src_idx2word={}
# src_vocab_size = len(src_vocab)
# for key,value in src_vocab.items():
#     src_idx2word[value] = key
#
# tgt_vocab = {
...
#target
# tgt_vocab=utils.word2indexofdec('dec_input.de')
# idx2word={}
# tgt_vocab_size= len(tgt_vocab)
...
f_enc_input = h5py.File("result_en.hdf5", "r")
src_vocab_size =f_enc_input['nword'][()]

f_dec_output = h5py.File("result_target.hdf5", "r")
tgt_vocab_size = f_dec_output['nword'][()]
# ==============================================================================================
# 数据构建
#
#
# def make_datas(sentences):
#     """把单词序列转换为数字序列"""
...
def make_data(i):
    f_enc_input = h5py.File("result_en.hdf5", "r")
    f_dec_input = h5py.File("result_de.hdf5", "r")
    f_dec_output = h5py.File("result_target.hdf5", "r")
    enc_inputs = torch.LongTensor(f_enc_input["group"][str(i)][:])
    # print('encinputs is :',enc_inputs,'end')
    # print('encinputsize is ',enc_inputs.size())
    dec_inputs = torch.LongTensor(f_dec_input["group"][str(i)][:])
    dec_outputs = torch.LongTensor(f_dec_output["group"][str(i)][:])
    return enc_inputs,dec_inputs,dec_outputs


# enc_inputs, dec_inputs, dec_outputs = make_data(1)
...
# loader = Data.DataLoader(MyDataSet(enc_inputs, dec_inputs, dec_outputs),11,True)
...
for epoch in range(epochs):
    for i in f_enc_input["group"]:
        enc_inputs, dec_inputs, dec_outputs = make_data(i)
        ...
        score = sentence_bleu(dec_outputs, output, weights=(0.25, 0.25, 0.25, 0.25))
